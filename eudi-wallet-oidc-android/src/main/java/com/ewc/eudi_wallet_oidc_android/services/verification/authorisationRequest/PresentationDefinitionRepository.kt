    ...
    suspend fun getPresentationDefinitionFromUri(uri: String?): PresentationDefinition? {
        ...
        return try {
            val result = safeApiCallResponse {
                ApiManager.api.getService()?.resolveUrl(uri)
            }
            ...
            result.fold(
                ...
                onFailure = { error ->
                    println("Error fetching presentation definition: ${error.message}")
                    null
                }
            )
        } catch (e: Exception) {
            println("Unexpected error fetching presentation definition: ${e.message}")
            null
        }
    }
}
